...
class AgentsForAmazonBedrock:
    ...
    def delete_agent(self, agent_name: str, delete_role_flag: bool = True) -> None:
        ...
        if delete_role_flag:
            ...
            try:
                ...
                print(
                    f"Error when deleting bedrock_kb_allow_policy from role: {_agent_role_name}\n{e}"
                )
            ...
            try:
                ...
                print(
                    f"Error when deleting bedrock_allow_policy from role: {_agent_role_name}\n{e}"
                )
            ...
            try:
                ...
                print(
                    f"Error when deleting role: {_agent_role_name} from agent: {agent_name}\n{e}"
                )
    ...
    def _create_agent_role(
        ...
        _agent_role_name = f"AmazonBedrockExecutionRoleForAgents_{agent_name}"
        _tmp_resources = [
            f"arn:aws:bedrock:{self._region}::foundation-model/{_model}"
            for _model in agent_foundation_models
        ]
    ...
    def add_action_group_with_lambda(
        ...
        _lambda_arn = self.create_lambda(
            agent_name, lambda_function_name, source_code_file, sub_agent_arns
        )
    ...
    def create_supervisor_agent(
        ...
        if kb_arns is not None:
            supervisor_instructions += (
                ". You also can take advantage of your available knowledge bases."
            )

        _supervisor_role_arn = self._create_agent_role(
            supervisor_agent_name, model_ids, kb_arns
        )
