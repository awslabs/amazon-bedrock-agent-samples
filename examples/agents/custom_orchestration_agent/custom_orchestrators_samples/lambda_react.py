...
def create_converse_api_prompt(context, messages):
    ...
    request = {
        "modelId": model_id,
        "system": [{"text": create_system_prompt(context)}],
        "messages": messages,
            ...
            for step in steps:
                if step:
                    ...
                    if orch_input.get("state") == "START":
                        messages.append(
                            message("user", {"text": orch_input.get("text")})
                        )
                    if orch_input.get("state") == "MODEL_INVOKED":
                        messages.append(
                            json.loads(orch_input.get("text")).get("output")
                        )
                    if orch_input.get("state") == "TOOL_INVOKED":
                        messages.append(
                            message("user", json.loads(orch_input.get("text")))
                        )
                    if (
                        orch_output.get("event") == "FINISH"
                        and orch_input.get("state") != "MODEL_INVOKED"
                    ):
                        messages.append(
                            message("assistant", json.loads(orch_output.get("text")))
                        )
...
def check_input_text(event):
    return (
        json.loads(event.get("input", {}).get("text", "{}")).get("text")
        == "send payload"
    )
