...
def get_specific_template_fields(contract_type):
    ...
    fields = {
        'purchase': [
            ...
            'purchase_price', 'closing_date', 'deposit_amount'
        ],
        'franchise': [
            'franchisor_name', 'franchisee_name', 'business_type',
            'brand_name', 'territory', 'initial_fee', 'royalty_rate'
        ],
        'timeandmaterial': [
            'contractor_name', 'client_name', 'scope_of_work',
            'labor_rates', 'markup_percentage', 'not_to_exceed_amount'
        ]
    }
...
def determine_contract_type(requirements):
    ...
    if any(term in requirements for term in ['purchase', 'buy', 'goods', 'product']):
        ...
        return 'franchise'
    elif any(term in requirements for term in ['consulting', 'hourly', 'time', 'material', 'service']):
