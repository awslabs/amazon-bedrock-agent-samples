...
- **[Not yet optimized models Agent](./not_yet_optimized_models_agent/)** This example refactors the Mortgage Assistant mult-agent example to work with both the Llama and Mistral models. These two models have not yet been optimized for Bedrock Agents, which means that some changes to the util folder files are needed to make the agents work and more prompting is needed to ensure tools and agents are called correctly. Users can contrast this folder to the Mortgage Assistant agent example to see what changes may be needed for their not yet optimized model
